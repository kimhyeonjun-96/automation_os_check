...
class JsonParsingService(ParsingService):
    ...
    def start_parsing(self):
        for data in self.dataList:
            host = Host()
            # if 'unreachable' not in data['hostinfo']['check_ssh']:
            if 'check_ssh' in data['hostinfo']:
                ...
                host.setUseRealMem(data['hostinfo']['ansible_memory_mb']['real'])
                ...
                # print("connect fail : {}".format(data['hostinfo']['inventory_hostname']))
                ...
                # host.setSshMsg(data['hostinfo']['ssh']['skip_reason'])
